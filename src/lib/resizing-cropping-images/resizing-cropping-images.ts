...
export class ResizingCroppingImagesComponent implements OnChanges, ControlValueAccessor {
    ...
    public moveImg($e: any) {
        $e.preventDefault();
        const containerHeight = this.elementRef.nativeElement.querySelector('.content-img').offsetHeight;
        const W = this.elementRef.nativeElement.querySelector('._img').offsetWidth;
        const H = this.elementRef.nativeElement.querySelector('._img').offsetHeight;
        const oTop = this.elementRef.nativeElement.querySelector('._img').offsetTop;
        if (this.stateType === 'move') {
            const topOffset = -offset(this.elementRef.nativeElement.querySelector('.content-img')).top +
                $e.clientY - (this.centerY);
            if (Math.abs(topOffset) <= H - containerHeight && topOffset <= 0) {
                this._top = topOffset;
            }
        } else if (this.stateType === 'resize') {
            this.resize($e, W, H, oTop, 0);
        }
    ...
    public resize(ev$: any, W: any, H: any, oTop: any, oLeft: any) {
        ...
        if (this.stateR === 'nw') {
            ...
            this.stateType = 'none';
            this.crop();
        } else if (this.stateR === 'init') {
            _W = Math.round(this.elementRef.nativeElement.querySelector('.content-img').offsetWidth);
            _H = H;
            this._left = 0;
            this._top = 0;
